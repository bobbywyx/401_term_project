...
def gamble_function(money_in_bet, info):
    ...
    if r < info["ruin_p"]:
        return 0
    elif r < info["p"]+info["ruin_p"]:
        return (1 + info["b"]) * money_in_bet
    else:
        return (1 - info["a"]) * money_in_bet


def decision_function(history_data, info):
    # return 0.1
    return c * (info["p"]/info["a"] - info["q"] / info["b"])
...
def calc_plot(bankroll_history_of_different_games):
    ...
    pi =c*( info["p"]/info["a"] - info["q"] / info["b"])
    print("pi:", pi)
    ...
    # set the title
    plt.title("p = %.1f, a= %.1f ,b = %.1f, $\pi$ = %.2f, z = %.2f" % (p,a, b, pi,ruin_p))
